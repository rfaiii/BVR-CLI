...
type UI struct {
	...
	// agentBusyCache / beastmodeCache memoize the workspace busy and permission
...
func New(com *common.Common, initialSessionID string, continueLast bool) *UI {
	...
	beastmode := com.Workspace.PermissionSkipRequests()
	ui.beastmodeCache.set(beastmode)
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.MouseClickMsg:
		...
		if m.dialog.HasDialogs() {
			if action := m.dialog.Update(msg); action != nil {
				if cmd := m.handleDialogAction(action); cmd != nil {
					cmds = append(cmds, cmd)
				}
			}
	...
	case tea.MouseWheelMsg:
		// Dialogs own scrolling while open (including the file finder).
		if m.dialog.HasDialogs() {
			m.dialog.Update(msg)
		}
		return m, tea.Batch(cmds...)
	...
	case uiFocusEditor:
		...
		if !m.bangMode && m.beastmodeModeCached() {
			m.textarea.Placeholder = "Beastmode!"
		}
	}
...
func (m *UI) handleKeyPressMsg(msg tea.KeyPressMsg) tea.Cmd {
	...
	handleGlobalKeys := func(msg tea.KeyPressMsg) bool {
		...
		case key.Matches(msg, m.keyMap.Suspend):
			...
			return true
		case key.Matches(msg, m.keyMap.ToggleBeastmode):
			beastmode := m.toggleBeastmodeMode()
			status := "disabled"
			if beastmode {
				status = "enabled"
			}
...
func (m *UI) bannerAnimation() string {
	if m.bannerAnim == nil {
		return ""
	}
	return m.bannerAnim.Render()
}
...
func (m *UI) setEditorPrompt(beastmode bool) {
	...
	if beastmode {
		m.textarea.SetPromptFunc(4, m.beastmodePromptFunc)
		return
	}
...
// beastmodePromptFunc returns the beastmode editor prompt style with warning icon
// and colored dots.
func (m *UI) beastmodePromptFunc(info textarea.PromptInfo) string {
	t := m.com.Styles
	if info.LineNumber == 0 {
		if info.Focused {
			return t.Editor.PromptBeastmodeIconFocused.Render()
		} else {
			return t.Editor.PromptBeastmodeIconBlurred.Render()
		}
	}
	if info.Focused {
		return t.Editor.PromptBeastmodeDotsFocused.Render()
	}
	return t.Editor.PromptBeastmodeDotsBlurred.Render()
}
...
// openFileBrowserDialog opens the in-app project file finder.
func (m *UI) openFileBrowserDialog() tea.Cmd {
	if m.dialog.ContainsDialog(dialog.FileBrowserID) {
		m.dialog.BringToFront(dialog.FileBrowserID)
		return nil
	}
	if m.fileBrowser == nil {
		browser, cmd := dialog.NewFileBrowser(m.com)
		m.fileBrowser = browser
...
func (m *UI) drawSessionDetails(scr uv.Screen, area uv.Rectangle) {
	...
	version := s.CompactDetails.Version.Width(width).AlignHorizontal(lipgloss.Right).Render(version.ShortVersion())
